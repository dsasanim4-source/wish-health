...
-- Replace this value with the Base32 secret shown in Google Authenticator setup.
...
begin
  digest_bytes := hmac(counter_bytes, key_bytes, 'sha1');
...
declare
  ...
  drift integer;
begin
  if p_secret is null or p_secret = '' or p_secret = 'REPLACE_WITH_YOUR_BASE32_SECRET' then
...
declare
  current_user public.app_users;
begin
  current_user := public.app_user_from_session(p_session_token);

  if current_user.password_hash <> crypt(p_current_password, current_user.password_hash) then
...
begin
  ...
  values (clean_username, nullif(trim(coalesce(p_display_name, '')), ''), crypt('123456', gen_salt('bf')), true)
...
begin
  ...
  return query
  select e.id, e.date, e.diet, e.mood, e.sleep, e.period, e.exercise, e.gratitude, e.created_at, e.updated_at
...
begin
  ...
  insert into public.daily_entries (
    id, user_id, date, diet, mood, sleep, period, exercise, gratitude, created_at, updated_at
  )
  values (
    entry_id,
    current_user.id,
...
begin
  ...
  return query
  select u.username, u.display_name, e.id, e.date, e.diet, e.mood, e.sleep, e.period, e.exercise, e.gratitude, e.created_at, e.updated_at
  from public.daily_entries e
