...
drop function if exists public.app_save_push_subscription(text, jsonb, text);
drop function if exists public.app_delete_push_subscription(text, text);
...
begin
  ...
  where id = p_user_id
  returning username into deleted_username;

  if deleted_username is null then
